CLASS zakp_itab_abap_sql DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zakp_itab_abap_sql IMPLEMENTATION.
...
* Example 1: Structured Variables in SELECT SINGLE ... INTO ... **********************************************************************
    ...
    out->write( data = airport_full name = `One of the airports in Zurich (Structure):` ).
...
* Example 5: UNION (ALL) **********************************************************************
    ...
    SELECT FROM /DMO/I_Airport
            ...
            WHERE City = 'London' ORDER BY type, Id INTO TABLE @DATA(names).
